...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="DPI used when rendering PDF pages to images before OCR. Default: 200.",
    )
    parser.add_argument(
        "--render-pdfs-first",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Render PDFs to page images with pypdfium2 before OCR. Default: enabled.",
    )
...
def render_pdf_to_images(input_file: Path, dpi: int) -> tuple[tempfile.TemporaryDirectory[str], list[Path]]:
    temp_dir = tempfile.TemporaryDirectory(prefix=f"{input_file.stem}_render_")
        ...
        for page_index in range(len(pdf)):
            ...
            try:
                ...
                try:
                    ...
                    page_path = temp_path / f"page-{page_index + 1:04d}.png"
    ...
    return temp_dir, rendered_paths
...
def process_document(
    ...
    print(f"[{time.time() - started_at:.1f}s] output_dir={output_dir}", flush=True)
    render_context: tempfile.TemporaryDirectory[str] | None = None

    try:
        if input_file.suffix.lower() == ".pdf" and args.render_pdfs_first:
            print(
                f"[{time.time() - started_at:.1f}s] rendering PDF pages first at {args.pdf_render_dpi} dpi",
                flush=True,
            )
            render_context, rendered_pages = render_pdf_to_images(input_file, args.pdf_render_dpi)
            print(
                f"[{time.time() - started_at:.1f}s] rendered {len(rendered_pages)} page image(s)",
                flush=True,
            )
            raw_pages = []
            for page_number, page_path in enumerate(rendered_pages, start=1):
                print(
                    f"[{time.time() - started_at:.1f}s] starting predict for rendered page {page_number}",
                    flush=True,
                )
                raw_pages.extend(list(pipeline.predict(input=str(page_path))))
        else:
            print(f"[{time.time() - started_at:.1f}s] starting predict", flush=True)
            raw_pages = list(pipeline.predict(input=str(input_file)))

        if not raw_pages:
            print(f"[empty] {input_file}", flush=True)
            return False
        print(f"[{time.time() - started_at:.1f}s] predict finished, pages={len(raw_pages)}", flush=True)
        print(f"[{time.time() - started_at:.1f}s] restructuring pages", flush=True)
        ...
        print(f"[{time.time() - started_at:.1f}s] restructuring finished", flush=True)
        ...
        for page_number, result in enumerate(pages_for_export, start=1):
            print(f"[{time.time() - started_at:.1f}s] processing page {page_number}", flush=True)
        ...
        print(f"[{time.time() - started_at:.1f}s] concatenating markdown", flush=True)
        merged_markdown = pipeline.concatenate_markdown_pages(markdown_pages)

        print(f"[{time.time() - started_at:.1f}s] writing json", flush=True)
        with json_path.open("w", encoding="utf-8") as handle:
            json.dump(merged_json, handle, ensure_ascii=False, indent=2)

        print(f"[{time.time() - started_at:.1f}s] writing markdown", flush=True)
        with markdown_path.open("w", encoding="utf-8") as handle:
            handle.write(merged_markdown)

        print(f"[{time.time() - started_at:.1f}s] saving {len(markdown_images)} images", flush=True)
        save_markdown_images(output_dir, markdown_images)
        print(
            f"[{time.time() - started_at:.1f}s] [done] {input_file} -> "
            f"{json_path.name}, {markdown_path.name}, images={len(markdown_images)}",
            flush=True,
        )
...
def main() -> int:
    ...
    print("Creating PaddleOCRVL pipeline...", flush=True)
    pipeline_started_at = time.time()
    pipeline = build_pipeline(args)
    print(f"Pipeline ready in {time.time() - pipeline_started_at:.1f}s.", flush=True)
